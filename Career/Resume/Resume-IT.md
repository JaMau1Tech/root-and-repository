# Ja'Maurian Smith-Williams

Winter Haven, FL | 727-422-4441 | jamaurwrk12@gmail.com | github.com/JaMau1Tech | linkedin.com/in/ja-maurian-smith-williams-46784741b

---

## Education

**Traviss Technical College — Computer Systems & Information Technology Program**
*Starting August 2026*

**Brandon High School — High School Diploma (Honors), Brandon, FL**
*Graduated with Honors — 4.8 GPA*

**TryHackMe — Pre Security (SEC0)**

---

## Technical Skills

- **Operating Systems:** Windows, Windows Server, Linux (Ubuntu)
- **Systems Administration:** Active Directory Domain Services, Domain Controllers, User & Group Management
- **IT Support:** Technical Troubleshooting, Hardware & Software Fundamentals, Command Line Basics
- **Networking:** Networking Fundamentals, IP Addressing, MAC Addresses, ICMP, Basic Network Troubleshooting
- **Programming:** Python (Beginner), JavaScript (Beginner), SQL (Beginner)
- **Version Control:** Git, GitHub
- **Documentation:** Markdown, Technical Documentation

---

## Technical Projects

**Active Directory & Windows Server Home Lab — Root & Repository**
- Built and configured a Windows Server domain controller, deploying Active Directory Domain Services for domain jamaursec.lab.
- Configured user accounts, security groups, and domain settings to replicate real-world enterprise IT infrastructure.
- Deployed and configured a Linux (Ubuntu) server, establishing baseline system administration and command-line proficiency.
- Documented each build step-by-step and version-controlled all configuration notes through Git and GitHub.

**Root & Repository — GitHub Technical Portfolio**
- Created and maintain a structured GitHub repository documenting IT support projects, system configurations, and troubleshooting notes.
- Implemented consistent documentation standards across all projects to support long-term readability and professional presentation.

---

## Professional Experience

**Sortation Associate — Amazon, Davenport, FL**
*Nov 2025 – Jul 2026*
- I processed up to 900+ of customer packages, on Inbound Dock, per shift across multiple sortation paths while maintaining accuracy and productivity standards.
- I operated handheld scanning technology and warehouse systems, on HIPPO Area, to track and stage shipments with minimal errors, kept a scan rate of 250+ packages every 15 minutes.
- I adapted quickly to shifting priorities and extended shifts, consistently meeting performance targets in a fast-paced fulfillment environment.

**Machine Operator — Coca-Cola Beverages Florida, Auburndale, FL**
*Jul 2025 – Oct 2025*
- I operated production equipment while performing quality and safety inspections that supported continuous, safe production output.
- I Identified and addressed equipment issues during shifts, helping maintain consistent production flow.
- I always maintained an organized, safety-compliant work area while collaborating with a team under physically demanding conditions.
